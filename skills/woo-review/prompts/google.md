...
If `/tmp/pr-review/chunks.txt` exists (issue #14), the outer loop iterates `(angle, chunk_id)` pairs. For each pair, read `/tmp/pr-review/diff.chunk-<id>.txt` and write findings to `/tmp/pr-review/findings.<angle>.<chunk_id>.json`. When `chunks.txt` is absent, fall back to `diff.txt` and `findings.<angle>.json`.
...
3. Write findings to `/tmp/pr-review/findings.<angle>.json` (or `findings.<angle>.<chunk_id>.json` in chunked mode) — JSON array conforming to `_header.md` schema.
