...
- **Chunked** (file present, issue #14): invoke **one `@generalist` per `(angle, chunk_id)` pair**, again in the same response. Pass the chunk id explicitly in the subagent brief and tell it to read `/tmp/pr-review/diff.chunk-<id>.txt` and write `/tmp/pr-review/findings.<angle>.chunk-<id>.json`.
...
You are the <angle> reviewer for this PR. Read:
  ...
  - /tmp/pr-review/diff.txt (or diff.chunk-<id>.txt when chunked)
  - /tmp/pr-review/meta.json

For `react` first run `npx -y react-doctor@$REACT_DOCTOR_VERSION --diff $BASE_REF --offline`.

Write findings as a JSON array to /tmp/pr-review/findings.<angle>.json
...
Spawn one `@generalist` with `$WOO_REVIEW_ACTION_PATH/prompts/validator-prosecutor.md` as its brief. It assumes each finding is real and drops only the clearly-wrong ones. It writes `/tmp/pr-review/findings.prosecutor.json` and EXITS — it MUST NOT post a review.
