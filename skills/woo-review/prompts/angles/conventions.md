...
**Output.** Write findings as a JSON array to `/tmp/pr-review/findings.conventions.json` using the schema in `_header.md`. Each finding gets `"angle": "conventions"` and MUST populate `title` (bold headline ≤60 chars), `description` (which rule is violated and where in the diff — no fix), `fix` (recommended change in prose), and `rule_quote` (verbatim from `rules.md`).
