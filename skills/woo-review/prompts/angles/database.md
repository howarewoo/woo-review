...
**Output.** Write findings as a JSON array to `/tmp/pr-review/findings.database.json` using the schema in `_header.md`. Each finding gets `"angle": "database"` and MUST populate `title` (bold headline ≤60 chars), `description` (issue + concrete impact path, no fix), and `fix` (mitigation in prose). Add `suggestion` only when a verbatim SQL or code replacement is safe. When citing a Supabase rule, quote its filename (e.g. `security-rls-basics.md`) in `rule_quote`.
